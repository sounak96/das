	...
	<div class="search" style="background-color:;">
		<h3 class="text-center" style="background-color:#272327;color: #fff;padding: 5px;">Customer Details</h3>
			...
			<?php 
					...
					$result = mysqli_query($conn,$sql);echo "string";
					...
					if($count>=1){
						...
						while($row=mysqli_fetch_array($result)){
								echo "<tr>";
								echo "<td>".decrypt($row['name'],$key)."</td>";
								echo "<td>".decrypt($row['age'],$key)."</td>";
								
								echo "<td>".decrypt($row['contact'],$key)."</td>";
								echo "<td>".decrypt($row['address'],$key)."</td>";
								
								echo "<td>".decrypt($row['bgroup'],$key)."</td>";
